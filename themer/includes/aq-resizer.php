    ...
    class WPCOM_Resize
    {
        ...
        public function process( $url, $width = null, $height = null, $crop = null, $img_id = 0, $size = '', $single = true, $upscale = false ) {
            try {
                ...
                // oss路径的图片使用oss图片服务返回
                if ( preg_match('/^oss:\/\//i', $upload_dir) || class_exists('WPOSS') || function_exists('oss_get_client') ) {
                    ...
                    return $image;
                }else if( (function_exists('wpcos_unique_filename') || defined('TENCENT_WORDPRESS_COS_VERSION') || function_exists('cos_get_client')) && !preg_match('/imageMogr2\//i', $upload_dir)) {
                    ...
                    return $image;
                }else if( (class_exists('WPQiNiu') || function_exists('kodo_get_default_options')) && !preg_match('/imageMogr2\//i', $upload_dir)) {
                    if($height) {
                    ...
                    return $image;
                }else if(function_exists('wpupyun_unique_filename') || function_exists('uss_get_client')){
                    if($height) {
                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        if ( ! is_wp_error( $resized_file ) ) {
                            ...
                            if(!$img_id){
                                ...
                                $img_id = $wpdb->get_var($wpdb->prepare("SELECT post_id FROM `$wpdb->postmeta` WHERE meta_key = '_wp_attached_file' AND meta_value = %s", $value));
                            }
                            ...
                            if($img_id){
                                ...
                                if(is_array($img_data)){
                                    ...
                                    if(is_array($img_data['sizes']) && isset($img_data['sizes'][$size]) && is_array($img_data['sizes'][$size]) && isset($img_data['sizes'][$size]['file'])){ // 重新生成了缩略图需要删除之前的缩略图片
                                        $delete = 0;
                                        // 老图片和新图片不同，初步考虑删除
