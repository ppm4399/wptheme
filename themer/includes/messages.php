...
if(!class_exists( Messages::class )):
    class Messages{
        ...
        function add_btn($user){
            echo '<button type="button" class="wpcom-btn btn-primary btn-xs btn-message j-message" data-user="'.$user.'">'.\WPCOM::icon('mail-fill', false).__('Message', 'wpcom').'</button>';
        }
        ...
        function get_messages($from, $to, int $last=0, int $num=10){
            global $wpdb;
            $table = $this->table;
            $where_id = '';
            $order = 'DESC';
            ...
            $results = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $table WHERE ((from_user = %d AND to_user = %d) OR (from_user = %d AND to_user = %d)) $where_id ORDER BY ID $order LIMIT %d", $from, $to, $to, $from, $num) );
        ...
        function get_message($id){
            global $wpdb;
            $results = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $this->table WHERE ID = %d", $id) );
        ...
        function get_messages_list($user, $num=10, $paged=1){
            ...
            $group = $wpdb->prepare("SELECT sum(case when status = '0' AND to_user = %d then 1 else 0 end) as unread, (case when from_user = %d then to_user else from_user end) as group_user, from_user, to_user FROM $this->table WHERE (to_user = %d OR from_user = %d) AND type = 'private' GROUP BY group_user", $user, $user, $user, $user);
            ...
            $results = $wpdb->get_results( $wpdb->prepare("SELECT T.ID, T.content, T.time, T.to_user, T.from_user, (case when T.status = '0' AND T.to_user = %d then '0' else '1' end) as status, L.unread, L.group_user FROM $this->table as T RIGHT JOIN ($list) as L ON T.ID = L.mid ORDER BY status ASC, T.ID DESC $limit", $user) );
        ...
        function get_messages_list_pages($user, $num=10){
            global $wpdb;
            $user = $user ?: get_current_user_id();
            $results = $wpdb->get_results($wpdb->prepare("SELECT (case when from_user = %d then to_user else from_user end) as group_user FROM $this->table WHERE (to_user = %d OR from_user = %d) AND type = 'private' GROUP BY group_user", $user, $user, $user));
        ...
        function admin_page(){ ?>
            <div class="wrap">
                ...
                <form method="post">
                    <?php
                    if ( ! class_exists( Messages_List::class ) ) require_once FRAMEWORK_PATH . '/includes/messages-list.php';
                    $list = new Messages_List();
                    $list->prepare_items();
