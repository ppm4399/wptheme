...
class WPCOM_DEMO_Importer{
    ...
    public function after_import( $selected_import ) {
        ...
        if($nav_menus && !empty($nav_menus)){
            foreach ( $nav_menus as $nav_menu ) {
                if($nav_menu && isset($nav_menu->term_taxonomy_id)) wp_update_term_count($nav_menu->term_taxonomy_id, 'nav_menu');
            }
        }
        ...
        if ($_wpcom_metas) {
            foreach ($_wpcom_metas as $metas) {
                ...
                if($meta_value && is_string($meta_value)) $meta_value = json_decode($meta_value, true);
    ...
    private function get_file_by_url( $url ){
        ...
        if( isset($matches[1]) && $matches[1] ){
            ...
            $file_id = $wpdb->get_var($wpdb->prepare("SELECT post_id FROM `$postmeta_table` WHERE meta_key = '_wp_attached_file' AND meta_value = %s", $name));
