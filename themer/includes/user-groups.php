...
class User_Groups {
    function __construct(){
        ...
        add_filter( 'wpcom_user_display_name', array( $this, 'display_name' ), 10, 3);
    }
    ...
    function register_user_groups(){
        ...
        $args = array(
            ...
            'labels' => $labels,
            'show_ui' => true,
            'show_in_rest' => current_user_can('publish_posts'),
    ...
    function user_groups_parent_file($parent_file = ''){
        ...
        if (!empty($_GET['taxonomy']) && ($_GET['taxonomy'] == 'user-groups') && ($pagenow == 'edit-tags.php' || $pagenow == 'term.php')) {
    ...
    function edit_user_groups($user){
        ...
        $terms = get_terms(array('taxonomy' => 'user-groups', 'hide_empty' => false)); ?>
    ...
    function display_name($name, $user_id, $type=''){
        if($type=='full'){
            $group = wpcom_get_user_group($user_id);
            if ( $group ) $name .= '<span class="user-group">'.$group->name.'</span>';
        }
        return $name;
    }
}