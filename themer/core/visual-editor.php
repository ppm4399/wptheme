...
class Visual_Editor{
    ...
    function page_modules(){
        ...
        $res = [];
        if($id && current_user_can( 'customize' ) && $modules = $this->get_modules($id)){
            $res = $modules;
        }
        $res = apply_filters('wpcom_visual_editor_preview_modules', $res, $id);
        wp_send_json($res);
    }

    function get_modules($id){
        $res = [];
        if($id && $modules = get_post_meta($id, '_page_modules', true)){
            $res = is_array($modules) && isset($modules['type']) ? [$modules] : $modules;
        }
        return apply_filters('wpcom_get_visual_editor_modules', $res, $id);
    }
    ...
    function get_module(){
        ...
        if(isset($_POST['id']) && $_POST['id'] && current_user_can( 'customize' )){
            $mds = $this->get_modules(sanitize_text_field($_POST['id']));
            $mid = isset($_POST['mid']) && $_POST['mid'] ? sanitize_text_field($_POST['mid']) : 0;
            if ($mds && is_array($mds)) {
                $data = [];
                foreach($mds as $i => $md){
    ...
    function render_page( $modules = null ){
        global $post;
        $render = $modules ?: $this->get_modules($post->ID);
        if(!$render) $render = [];
    ...
    public function reset_module_id($module, $mid){
        ...
        } else if ($module['settings'] && isset($module['settings']['grids']) && $module['settings']['grids']) {
            foreach ($module['settings']['grids'] as $b => $grids) {
                foreach ($grids as $c => $grid) {
                    $module['settings']['grids'][$b][$c] = $this->reset_module_id($grid, $mid);
                }
            }
        } else if ($module['settings'] && $module['type'] === 'tabs' && isset($module['settings']['tabs']) && $module['settings']['tabs']) {
            foreach ($module['settings']['tabs'] as $d => $tabs) {
                foreach ($tabs as $e => $tab) {
                    $module['settings']['tabs'][$d][$e] = $this->reset_module_id($tab, $mid);
                }
            }
        }
        return $module;
    ...
    public function enqueue_module_script(){
        ...
        if( is_singular() && (is_page_template('page-home.php') || is_singular('page_module')) ) {
            $modules = $this->get_modules($post->ID);
            if( !$modules ) $modules = [];
            if(isset($modules['type'])) $modules = [ $modules ];
    ...
    private function loop_module_script($module, $onlyjs = 0){
        ...
        $_module_scripts = $_module_scripts ?: [];
        ...
        if ($module['settings'] && isset($module['settings']['grids']) && $module['settings']['grids']) {
            foreach ($module['settings']['grids'] as $grids) {
                foreach ($grids as $grid) {
                    if (isset($module['settings']['parent-id'])) $grid['settings']['parent-id'] = $module['settings']['parent-id'];
                    $this->loop_module_script($grid, $onlyjs);
                }
            }
        } else if ($module['settings'] && $module['type'] === 'tabs' && isset($module['settings']['tabs']) && $module['settings']['tabs']) {
            foreach ($module['settings']['tabs'] as $tabs) {
                foreach ($tabs as $tab) {
                    if (isset($module['settings']['parent-id'])) $tab['settings']['parent-id'] = $module['settings']['parent-id'];
                    $this->loop_module_script($tab, $onlyjs);
                }
            }
        }
    ...
    function body_class($classes){
        return array_merge( $classes, ['visual-editor'] );
    }
    ...
    function admin_bar_item() {
        ...
        if($post && $post->ID && get_queried_object_id() == $post->ID && get_page_template_slug($post->ID) == 'page-home.php') {
            ...
            $wp_admin_bar->add_node(array(
                'id' => 've-link',
                'title' => '<span class="ab-icon"><svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg