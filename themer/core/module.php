...
namespace WPCOM\Modules {
    class Module{
        ...
        function __construct($id, $name, $options = [], $icon='', $thumb = '', $cache = true) {
            $this->id = $id;
            $this->name = $name;
        ...
        function animate($atts){
            ...
            if(isset($atts['animate']) && $atts['animate']){
                ...
                if(isset($_animate->animate) && $_animate->animate){
                    $animate = ' data-anm="'.esc_attr($_animate->animate.', '.(isset($_animate->duration) ? $_animate->duration : '')).'"';
                }
            }
            return $animate;
        }

        function animate_item($delay = 60){
            $animate = '';
            if($this->value('animate')){
                $_animate = json_decode($this->value('animate'));
                if(isset($_animate->animate) && $_animate->animate){
                    $animate = ' data-anm="'.esc_attr($_animate->animate.', '.(isset($_animate->duration) ? $_animate->duration : '').', '.$delay).'"';
                }
            }
            return $animate;
        }
        ...
        function get_style($atts){
            ...
            $style = ($default ?: []) + (is_array($style) ? $style : []);
        ...
        function style($atts){ return [];}
        function style_inline($atts){}

        public function is_preview(){
            return did_action('visual_editor_preview_init') > 0;
        }
    ...
    class_alias(Module::class, 'WPCOM_Module');
}
...
namespace {
    if( !function_exists('register_module') ){
        function register_module( $module_class ){
            ...
            if(!isset($_wpcom_modules)) $_wpcom_modules = [];
        ...
        function wpcom_init_module(){
            ...
            if(!isset($wpcom_modules)) $wpcom_modules = [];
