...
function filter_content($content=''){
    ...
    if(count($title)>0){
        for($i=0; $i<count($matches[0]); $i++){
            $f1 = preg_quote($matches[0][$i], '/');
            $f2 = isset($matches[0][$i+1]) ? preg_quote($matches[0][$i+1], '/') : '$';
            preg_match('/('.$f1.')([\s\S]*?)('.$f2.')/i', $content, $matches2);
            $sec[] = $matches2[2];
        }
        $html .='<ul class="entry-tab j-tabs clearfix">';
        for($x=0; $x<count($title); $x++){
            $html .='<li class="entry-tab-item j-tabs-item'.($x==0?' active':'').' i-'.$x.'">'.$title[$x].'</li>';
        }
        $html .= '</ul>';
        for($y=0; $y<count($sec); $y++){
            $html .='<div class="entry-tab-content j-tabs-wrap'.($y==0?' active':'').' i-'.$y.'">'.$sec[$y].'</div>';
        }
...
function the_content_filter_images( $content ) {
    if(function_exists('WWA_is_rest') && WWA_is_rest()) return $content;
    if ( is_feed() || is_admin()
    ...
    foreach ( $matches[0] as $imgHTML ) {
        ...
        if( WPCOM::is_spider() ) {
            ...
            if(!isset($options['post_img_lazyload']) || $options['post_img_lazyload']=='1'){
                ...
                $replaceHTML = preg_replace( '/<img(.*?)src=([\'"])/i', '<img${1}src=${2}' . $placeholder_image . '${2} data-original=${2}', $imgHTML );
...
function filter_images_add_class( $image, $newClass ) {
    ...
    if ( preg_match( $pattern, $image, $matches ) ) {
        ...
        $image = preg_replace( '/(\<.+\s)src=([\'"])(.+\s)/', sprintf( '${1}class=${2}%s${2} src=${2}${3}', $newClass ), $image );
    }
    return $image;
}
...
function filter_images_add_alt( $image = '', $alt='' ){
    ...
    if ( preg_match( $pattern, $image, $matches ) ) {
        if ( ! trim($matches[1]) ) {
            $image = preg_replace( '/(\<.+\s)alt=([\'"])(.+)/', sprintf( '${1}alt=${2}%s${3}', $alt ), $image );
        }
    } else { // alt attribute not set.
        $image = preg_replace( '/(\<.+\s)src=([\'"])(.+)/', sprintf( '${1}alt=${2}%s${2} src=${2}${3}', $alt ), $image );
    }
    return $image;
}
...
function wpcom_keyword_link( $content, $page = false ){
    global $options;
    if( isset($options['kl_keyword']) && $options['kl_keyword'] && $options['kl_keyword'][0] ) {
        foreach ($options['kl_keyword'] as $i => $keyword){
            ...
            $_url = explode(', ', $options['kl_link'][$i]);
            ...
            $url .= '<a '.WPCOM::url($options['kl_link'][$i]).' title="'.esc_attr($options['kl_title'][$i]?$options['kl_title'][$i]:$keyword).'"';
            ...
            //ignore a
            if ($a_num = preg_match_all("/<a\s.*?>.*?<\/a>/is", $content, $ignore_a))
                for ($i = 1; $i <= $a_num; $i++)
                    $content = preg_replace("/<a\s.*?>.*?<\/a>/is", "%ignore_a_$i%", $content, 1);
            ...
            if ($ignore_a) {
                for ($i = 1; $i <= $a_num; $i++) {
                    $content = str_replace("%ignore_a_$i%", $ignore_a[0][$i - 1], $content);
                }
            }
        }
    }
...
add_filter( 'the_content', 'wpcom_remove_nl', 100 );
function wpcom_remove_nl( $content ){
    preg_match_all( '/<p(?![a-zA-Z]).*?>(.*?)<\/p>/is', $content, $matches );
    $search = [];
    $replace = [];
    foreach ( $matches[0] as $pHTML ) {
        if( in_array($pHTML, $search) ){ continue; }
        $replaceHTML = $pHTML;
        $replaceHTML = preg_replace( '/(\r\n|\n)+/', '', $replaceHTML );
        array_push( $search, $pHTML );
        array_push( $replace, $replaceHTML );
    }
    $content = str_replace( $search, $replace, $content );
    return $content;
